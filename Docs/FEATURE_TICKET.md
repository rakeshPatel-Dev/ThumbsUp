...
| **Edit/Delete Pending Task** | Yes | No | No | Only allowed if status is `pending`. |
| **Review & Approve/Reject Tasks** | No | Yes | Yes | Managers transition tasks to `approved` or `rejected`. |
| **Mark Task Completed** | Yes | No | No | Employees mark their own `approved` tasks as `completed`. |
| **Access Drag-and-Drop Board** | No | No | Yes | Admin Kanban board to manage task statuses. |
...
3. **Decision**:
   - **Approval**: If a **Manager** or **Admin** clicks **Approve**, the task status shifts to **Approved**.
   - **Rejection**: If a **Manager** or **Admin** clicks **Reject**, the status shifts to **Rejected**.
4. **Resolution**:
   - For **Approved** tasks: The **Employee** works on the task. Once finished, they click **Mark Complete**, shifting the state to **Completed**.
   - For **Rejected** tasks: The **Employee** can edit the task (e.g., correcting description/details), which automatically submits it back to the **Pending** state for review.
...
### 4.3 Admin Tools
- **Drag-and-Drop Kanban Board**: Admins can drag task cards between Pending, Approved, Rejected, and Completed columns. Relies on `@dnd-kit` for native pointer sensor dragging.
